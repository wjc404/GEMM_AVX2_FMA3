...
gcc -fopenmp sgemmtest.c -Wl,--start-group /opt/intel/mkl/lib/intel64/libmkl_intel_lp64.a /opt/intel/mkl/lib/intel64/libmkl_gnu_thread.a /opt/intel/mkl/lib/intel64/libmkl_core.a -Wl,--end-group -lpthread -lm -ldl -o sgemmtest
*/

int main(int argc, char* argv[]) // command line: ./sgemmtest [niter] [m] [n] [k] [upperbound] [lowerbound] [transa] [transb]
{
...
// start C = 2AB
	alpha = 2.0;beta = -1.0;
        ...
        if(niters < 100) printf("First 5 elements of product matrix 2(AB)mkl: %e, %e, %e, %e, %e\n",C[0],C[1],C[2],C[3],C[4]);//DEBUG
// start C = 2C - 4AB
	alpha = -4.0;beta = 2.0;
        gettimeofday(&starttime,0);
        ...
        if(niters < 100) printf("First 5 elements of matrix 4(AB)mkl-4(AB)test: %e, %e, %e, %e, %e\n",C[0],C[1],C[2],C[3],C[4]);//DEBUG
